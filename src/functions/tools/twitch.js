...
module.exports = (client) => {
  ...
  client.checkLive = async () => {
    ...
    let Zroolchannel = client.channels.cache.get("1017088259169456238");
    let twitchchannel = client.channels.cache.get(config.twitchchannel);
            ...
            if (stream.type === "live") {
              if (
                ...
                axios(getURLData).then(async function (iconData) {
                  ...
                  if (
                    stream.user_name == "Zrool" ||
                    stream.user_login == "zrool"
                  )
                    twitchchannel = Zroolchannel;
